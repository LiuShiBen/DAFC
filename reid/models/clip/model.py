...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(self, x: torch.Tensor, prompt: torch.Tensor):
        self.attn_mask = self.attn_mask.to(dtype=x.dtype, device=x.device) if self.attn_mask is not None else None

        if prompt is not None:
            pk, pv = prompt
            pk, pv = pk.permute(1, 0, 2), pv.permute(1, 0, 2)
            #print(pk.shape, pv.shape, x.shape)
            k = torch.cat((pk, x), dim=0)
            v = torch.cat((pv, x), dim=0)
        else:
            k = x
            v = x
...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, prompt=None, image_query=None, text_prompt=None):
        for i,blk in enumerate(self.resblocks):
            if prompt is not None:
                if i == 0 :
                    #print("text_query:", text_query.shape)
                    p_list, x = prompt.forward(image_query, text_prompt, i, x)
                else:
                    image_query = x[0,:,:]
                    #print("q", q.shape, x.shape, i)
                    p_list, x = prompt.forward(image_query, text_prompt, i, x)
                #prompt_loss += loss
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, prompt=None, image_query=None, text_prompt=None):
        ...
        x = x.permute(1, 0, 2)  # NLD -> LND    #129,128,768
        x12 = self.transformer(x, prompt, image_query, text_prompt)
